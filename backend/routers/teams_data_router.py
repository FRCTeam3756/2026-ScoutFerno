...
async def import_statbotics_team_data_route(
    ...
    team_profile = await get_team_profile(team_number)
    team_event_data = await get_team_data(team_number, competition)
    team_data = await build_team_data_from_statbotics(
        team_number,
        competition,
        team_profile,
        team_event_data
