...
async def delete_postmatch_data_by_match(competition: str, match_number: int, flagError: bool = True):
    with Session(team_engine) as session:
        statement = select(Postmatch_Data).where(
            Postmatch_Data.competition == competition,
            Postmatch_Data.match_number == match_number
...
async def delete_postmatch_data_by_team(team_number: int, flagError: bool = True):
    with Session(team_engine) as session:
        statement = select(Postmatch_Data).where(
            Postmatch_Data.team_number == team_number,
        )
...
async def delete_postmatch_data_by_team_match(competition: str, match_number: int, team_number: int, flagError: bool = True):
    with Session(team_engine) as session:
        statement = select(Postmatch_Data).where(
            Postmatch_Data.competition == competition,
            Postmatch_Data.match_number == match_number,
