...
@router.get("/teleop_data/match/{match_number}", response_model=list[Teleop_Data])
# async def read_teleop_data_by_match_route(match_number: int, creds: Credentials = Depends(require_auth)):
async def read_teleop_data_by_match_route(match_number: int):
    return await read_teleop_data_by_match(match_number)


@router.get("/teleop_data/team/{team_number}/match/{match_number}", response_model=list[Teleop_Data])
# async def read_teleop_data_by_team_match_route(team_number: int, match_number: int, creds: Credentials = Depends(require_auth)):
async def read_teleop_data_by_team_match_route(team_number: int, match_number: int):
    return await read_teleop_data_by_team_match(team_number, match_number)


@router.patch("/teleop_data/team/{team_number}/match/{match_number}", response_model=Teleop_Data)
