...
@router.get("/teleop_data/year/{year}/competition/{competition}/match/{match_type}/{match_number}", response_model=list[Teleop_Data])
# async def read_teleop_data_by_match_route(match_number: int, creds: Credentials = Depends(require_auth)):
async def read_teleop_data_by_match_route(year: int, competition: str, match_type: str, match_number: int):
    return await read_teleop_data_by_match(year, competition, match_type, match_number)


@router.get("/teleop_data/year/{year}/competition/{competition}/team/{team_number}/match/{match_type}/{match_number}", response_model=list[Teleop_Data])
# async def read_teleop_data_by_team_match_route(team_number: int, match_number: int, creds: Credentials = Depends(require_auth)):
async def read_teleop_data_by_team_match_route(year: int, competition: str, team_number: int, match_type: str, match_number: int):
    return await read_teleop_data_by_team_match(year, competition, team_number, match_type, match_number)


@router.patch("/teleop_data/year/{year}/competition/{competition}/team/{team_number}/match/{match_type}/{match_number}", response_model=Teleop_Data)
...
async def update_teleop_data_route(year: int, competition: str, team_number: int, match_type: str, match_number: int, match_data: Teleop_Data_Update):
    return await update_teleop_data(year, competition, team_number, match_type, match_number, match_data)


@router.delete("/teleop_data/year/{year}/competition/{competition}/team/{team_number}/match/{match_type}/{match_number}")
# async def delete_match_teleop_data_route(team_number: int, match_number: int, creds: Credentials = Depends(require_auth)):
async def delete_match_teleop_data_route(year: int, competition: str, team_number: int, match_type: str, match_number: int):
    return await delete_match_teleop_data(year, competition, team_number, match_type, match_number)
